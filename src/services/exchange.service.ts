...
export default class TransactionService{
    ...
    public async buy(wallet_id_with: string, coin_to_buy: string, amount: number, wallet_id_to?: string){
        ...
        if(!wallet_id_to){
            wallet_id_to = await this.walletService.createWallet(wallet_id_with, coin_to_buy);
        }
        // @ts-ignore
    ...
    public async sell(wallet_id_with: string, wallet_id_to: string, coin_to_sell: string, amount: number){
        const price = await this.pricingService.getPricingFromCoinbase(coin_to_sell);
